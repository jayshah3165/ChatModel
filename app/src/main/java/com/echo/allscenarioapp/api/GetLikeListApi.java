...
public class GetLikeListApi {

    private final static String API = Const.GET_LIKE_LIST_API;
    private ResponseListener responseListener = null;
    private Context context = null;
    private static IRequestData iRequestData = null;
    public ArrayList<LikeListModel> objList = null;

    public GetLikeListApi(Context _context, ResponseListener _responseListener) {
    ...
    private interface IRequestData {
        @FormUrlEncoded
        @POST(API)
        Call<ResponseModel<LikeListModel>> getResponseData(
    ...
    public Void execute(String post_id,String types) {


       try {

            Call<ResponseModel<LikeListModel>> call = iRequestData.getResponseData(Pref.getStringValue(context, Const.PREF_USERID,""),post_id,types, Const.DEVICE_TYPE, Pref.getStringValue(context, Const.PREF_GCMTOKEN,""), Pref.getStringValue(context, Const.PREF_LANGUAGE, "EN"));
            call.enqueue(new Callback<ResponseModel<LikeListModel>>() {


                @Override
                public void onResponse(Call<ResponseModel<LikeListModel>> call, Response<ResponseModel<LikeListModel>> response) {
                    int status = 0;
                    String mesg = "";
                    try {
                        ...
                        objList = new ArrayList<LikeListModel>();
                        if (status == 1) {
                ...
                @Override
                public void onFailure(Call<ResponseModel<LikeListModel>> call, Throwable t) {
